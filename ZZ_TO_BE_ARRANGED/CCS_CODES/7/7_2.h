...
#DEFINE VALUE_SIDE_CURRENT_AIR                                 70
...
#DEFINE DATA_AIR_CONDITIONING_ON                               0xA0
#DEFINE DATA_CONFIRM_AIR_CONDITIONING_ON                       0xA1
#DEFINE DATA_AIR_CONDITIONING_OFF                              0xA2
#DEFINE DATA_CONFIRM_AIR_CONDITIONING_OFF                      0xA3
//******************************************************************************************************************************FAN**DRIVER**PINs**//
#DEFINE FAN_DRIVER_STEP_1                                      PIN_B1
#DEFINE FAN_DRIVER_STEP_2                                      PIN_B2
#DEFINE FAN_DRIVER_STEP_3                                      PIN_B0
//***************************************************************************************************************************FAN**PASSENGER**PINs**//
#DEFINE FAN_PASSENGER_STEP_1                                   PIN_B4
#DEFINE FAN_PASSENGER_STEP_2                                   PIN_B3
//**************************************************************************************************************************************CAM**Pins**//
...
//*********************************************************************************************************************************KLIMA-POMP_OUT**//
#DEFINE AIR_CONDITIONING_OUT                                   PIN_C3
#DEFINE POMP_OUT                                               PIN_B5  
#DEFINE AIR_CONDITIONING_OUT_ON_INFO                           PIN_D4
//*****************************************************************************************************************************TIME**POMP**VALUEs**//
...
BOOLEAN state_air_conditioning_out_on_info_once                = FALSE;
BOOLEAN state_confirm_air_conditioning_on                      = FALSE;
BOOLEAN state_confirm_air_conditioning_off                     = FALSE;
//*************************************************************************************************************************************************//
//*******************************************************************SETTING***********************************************************************//
//*************************************************************************************************************************************************//
VOID Setting(){
...
//*************************************************************************************************************************************COUNTER**0**//
      IF((counter_fan_passenger % 3) == 0){
         ...
         OUTPUT_LOW(FAN_PASSENGER_STEP_1);
         OUTPUT_LOW(FAN_PASSENGER_STEP_2);
      }
//*************************************************************************************************************************************COUNTER**1**//
      IF((counter_fan_passenger % 3) == 1){
         counter_fan_passenger_old     = counter_fan_passenger;
         OUTPUT_HIGH(FAN_PASSENGER_STEP_1);
         OUTPUT_LOW(FAN_PASSENGER_STEP_2);
      }
//*************************************************************************************************************************************COUNTER**2**//
      IF((counter_fan_passenger % 3) == 2){
         counter_fan_passenger_old     = counter_fan_passenger;
         OUTPUT_HIGH(FAN_PASSENGER_STEP_1);
         OUTPUT_HIGH(FAN_PASSENGER_STEP_2);
      }
   }
//*************************************************************************************************************************************************//
//*******************************************************************CAM**CONTROL******************************************************************//
...
//*************************************************************************************************************************************************//
                  ...
                  time_window_difference                          = Ticks_Difference(time_window_starting , time_window_current);
                  Turn_Right_Window();
                  IF(time_window_difference > (time_window_mid + (500 * 1000))){
                     Stop_Window();
                     Time_Delay(20);
                     state_turn_complete_window                   = TRUE;
...
//**************************************************************************************************************************ARKA**FAN**COUNTER**0**//
      IF(data_receive == DATA_FAN_PASSENGER_0){
         counter_fan_passenger = 0;
         DELAY_MS(20);
         Data_Sender(DATA_CONFIRM_FAN_PASSENGER_0);
         data_receive = DATA_CHECK;
      }
//**************************************************************************************************************************ARKA**FAN**COUNTER**1**//
      IF(data_receive == DATA_FAN_PASSENGER_1){
         counter_fan_passenger = 1;
         DELAY_MS(20);
         Data_Sender(DATA_CONFIRM_FAN_PASSENGER_1);
         data_receive = DATA_CHECK;
      }
//**************************************************************************************************************************ARKA**FAN**COUNTER**2**//
      IF(data_receive == DATA_FAN_PASSENGER_2){
         counter_fan_passenger = 2;
         DELAY_MS(20);
         Data_Sender(DATA_CONFIRM_FAN_PASSENGER_2);
         data_receive = DATA_CHECK;
      }
//********************************************************************************************************************************CAM**COUNTER**0**//
...
//***********************************************************************************************************************************FIX**ERROR****//
      ...
      IF(data_receive == DATA_CONFIRM_AIR_CONDITIONING_ON){
         state_confirm_air_conditioning_on = TRUE;
      }
      IF(data_receive == DATA_CONFIRM_AIR_CONDITIONING_OFF){
         state_confirm_air_conditioning_off = TRUE;
      }
}
//*************************************************************************************************************************************************//
//********************************************************************MAKE**A**DELAY***************************************************************//
...
VOID Assign_Controller_Klape_Error_According_To_Coming_Data(){
   ...
   IF(data_receive == DATA_FAN_PASSENGER_0){
      counter_fan_passenger = 0;
      DELAY_MS(20);
      Data_Sender(DATA_CONFIRM_EEPROM_FAN_PASSENGER);
      data_receive = DATA_CHECK;
   }
   IF(data_receive == DATA_WINDOW){
      counter_window = 0;
...
//*************************************************************************************************************************************************//
               state_confirm_air_conditioning_on               = FALSE;
               WHILE(!state_confirm_air_conditioning_on){
                  Data_Sender(DATA_AIR_CONDITIONING_ON);
                  Time_Delay(50);
               }
...
//*************************************************************************************************************************************************//
            state_confirm_air_conditioning_off               = FALSE;
            WHILE(!state_confirm_air_conditioning_off){
               Data_Sender(DATA_AIR_CONDITIONING_OFF);
               Time_Delay(50);
            }
